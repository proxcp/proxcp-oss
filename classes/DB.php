...
class DB {
	...
	private function __construct() {
		try {
			$this->_pdo = new PDO(Config::get('database/type') . ':host=' .
	...
	public function limit_action($action, $table, $where = array(), $limit) {
		if(count($where) === 3) {
			$operators = array('=', '>', '<', '>=', '<=', '!=');
	...
	public function desc_limit_action($action, $table, $where = array(), $limit) {
		if(count($where) === 3) {
			$operators = array('=', '>', '<', '>=', '<=', '!=');
			...
			if(in_array($operator, $operators)) {
				$sql = "{$action} FROM {$table} WHERE {$field} {$operator} ? ORDER BY `date` DESC LIMIT {$limit}";
				if(!$this->query($sql, array($value))->error()) {
					return $this;
				}
			}
	...
	public function asc_user_action($action, $table, $where = array()) {
		if(count($where) === 3) {
			$operators = array('=', '>', '<', '>=', '<=', '!=');
			...
			if(in_array($operator, $operators)) {
				$sql = "{$action} FROM {$table} WHERE {$field} {$operator} ? ORDER BY `username` ASC";
				if(!$this->query($sql, array($value))->error()) {
					return $this;
				}
			}
	...
	public function update_iso($table, $id, $fields) {
		...
		$sql = "UPDATE {$table} SET {$set} WHERE upload_key = '{$id}'";
	...
	public function update_dhcp($table, $id, $fields) {
		...
		$sql = "UPDATE {$table} SET {$set} WHERE ip = '{$id}'";
	...
	public function updatevm_aid($table, $aid, $fields) {
		...
		$sql = "UPDATE {$table} SET {$set} WHERE hb_account_id = {$aid}";
	...
	public function update_address($table, $aid, $fields) {
		...
		$sql = "UPDATE {$table} SET {$set} WHERE address = '{$aid}'";
	...
	public function updatevm_clid($table, $aid, $fields) {
		...
		$sql = "UPDATE {$table} SET {$set} WHERE cloud_account_id = {$aid}";
	...
	public function updatevnc($table, $id, $fields) {
		...
		$sql = "UPDATE {$table} SET {$set} WHERE hb_account_id = {$id}";
