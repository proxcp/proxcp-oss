...
if(!empty($_FILES) && $validation->passed() && $_POST['useriso_type'] != 'default') {
  ...
  try {
    ...
    if($status == 'new') {
      $today = new DateTime();
      $db->insert('vncp_kvm_isos_custom', array(
        'user_id' => (int)$_POST['useriso_who'],
        'fname' => $_POST['useriso_fname'],
        'type' => $_POST['useriso_type'],
        'rname' => $fileMeta['name'],
        'upload_date' => $today->format('Y-m-d 00:00:00'),
        'status' => 'created',
